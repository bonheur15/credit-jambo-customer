...
export type CreateDeviceVerificationInput = z.infer<
  typeof insertDeviceVerificationSchema
>;

export async function createDeviceVerification(
  data: CreateDeviceVerificationInput,
) {
...
export async function findDeviceVerificationByDeviceId(deviceId: string) {
  const result = await db
    ...
    .where(eq(deviceVerifications.device_id, deviceId));
  return result[0];
}
