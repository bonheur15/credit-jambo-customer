...
function TransactionsComponent() {
	...
	const fetchAccounts = async () => {
		...
		if (!token) {
			// This should ideally not happen if useEffect handles redirection, but as a fallback
	...
	const fetchTransactions = async (accounts: TAccount[]) => {
		...
		for (const account of accounts) {
			try {
				const response = await fetch(`${API_URL}accounts/${account.id}/transactions`, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				});
		...
		<div className="flex min-h-screen max-w-[700px] mx-auto w-full flex-col bg-white font-sans dark:bg-zinc-900">
					...
					<div className="mt-4 space-y-5">
						{transactions.length === 0 ? (
							...
							transactions.map((tx) => {
								...
								const accountCurrency = accounts.find((a) => a.id === tx.account_id)?.currency || "";
								return (
									<div key={tx.id} className="flex items-center justify-between">
										...
										>
											{isPositive ? `+${accountCurrency} ${tx.amount}` : `-${accountCurrency} ${Math.abs(tx.amount)}`}
										</p>
									</div>
								);
							})
			...
			<nav className="fixed bottom-0 left-0 w-full border-t border-zinc-200 bg-white px-6 py-4 dark:border-zinc-700 dark:bg-zinc-900">
				<div className="mx-auto flex max-w-sm justify-around">
					<button className="text-zinc-400 dark:text-zinc-500" onClick={() => navigate({ to: "/dashboard" })}>
						<HomeIcon className="h-7 w-7" />
					</button>
					<button className="text-zinc-900 dark:text-zinc-100" onClick={() => navigate({ to: "/transactions" })}>
